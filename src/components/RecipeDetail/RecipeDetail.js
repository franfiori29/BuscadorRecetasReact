...
export default function RecipeDetail({ id, recipe }) {
    ...
    console.log(recipe)
    if (Object.keys(recipe).length < 4) { <h1>Loading</h1> }

    return (
        <div style={{ textAlign: "center", width: "80%", margin: "0 auto", position: "relative" }} >
            <Link to="/recipes">
                <Icon color="black" style={{ position: "absolute", left: '10px' }} name="arrow left" size="big" />
            </Link>
            <h1 style={{ fontSize: '40px' }}>{recipe.title}</h1>
            {recipe && recipe.diets.map(rec =>
                <Label color='teal'>
                    {rec.toUpperCase()}
                </Label>
            )} <br />
            <div style={{ display: 'flex', flexWrap: 'wrap', margin: '20px 0' }}>
                ...
                <List style={{
                    ...
                    <List.Header content="INGREDIENTS" style={{ fontWeight: '700', fontSize: '30px', margin: "50px 0" }} />
                    {recipe.extendedIngredients.map(ing => (
                        <List.Item style={{ fontSize: '20px', margin: '5px 0' }}>
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Icon name="like"
                        ...
                        onMouseOut={() => setHover("")}
                        color={localStorage.getItem(recipe.id) ? "red" : ""}
                        onClick={handleClick}
            ...
            <p style={{ fontSize: '20px' }} dangerouslySetInnerHTML={{ __html: `${recipe.summary}` }}></p>
